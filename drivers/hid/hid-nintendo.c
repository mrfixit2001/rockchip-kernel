...
/* Default report to be used when HID descriptor is missing */
static u8 jc_desc[] = {
...
static void joycon_player_led_brightness_set(struct led_classdev *led,
...
static void joycon_home_led_brightness_set(struct led_classdev *led,
					  enum led_brightness brightness)
{
	struct device *dev = led->dev->parent;
	struct hid_device *hdev = container_of(dev, struct hid_device, dev);
	...
	mutex_lock(&ctlr->output_mutex);
	joycon_send_subcmd(ctlr, req, 5, HZ/4);
	mutex_unlock(&ctlr->output_mutex);
}
...
static int joycon_read_info(struct joycon_ctlr *ctlr)
{
	...
	/* MRFIXIT: if we know the controller type, set it. Otherwise pull it from the report */
	if(ctlr->hdev->product == USB_DEVICE_ID_NINTENDO_JOYCONL)
		ctlr->ctlr_type = JOYCON_CTLR_TYPE_JCL;
	else if(ctlr->hdev->product == USB_DEVICE_ID_NINTENDO_JOYCONR)
		ctlr->ctlr_type = JOYCON_CTLR_TYPE_JCR;
	else
...
static inline u8* nintendo_hid_report_fixup(struct hid_device *hdev, u8 *buf, unsigned int *size)
{
