...
#define LOG(x...)   printk(KERN_INFO "[BCM4359]: "x)
#define BT_UPORT 4

// extern s3c_wake_peer_t s3c2410_serial_wake_peer[CONFIG_SERIAL_8250_NR_UARTS];
...
static const struct of_device_id bcm4359_bluetooth_match[] = {
	{
		.compatible = "broadcom,bcm4359",
	},
	{},
};
MODULE_DEVICE_TABLE(of, bcm4359_bluetooth_match);
...
static struct platform_driver bcm4359_bluetooth_platform_driver = {
	...
	.driver = {
		   ...
		   .owner = THIS_MODULE,
		   .of_match_table = bcm4359_bluetooth_match,
		   },
};
...
static int __init bcm4359_init(void)
{
	LOG("Enter %s\n", __func__);
	return platform_driver_register(&bcm4359_bluetooth_platform_driver);
}

static void __exit bcm4359_exit(void)
{
	LOG("Enter %s\n", __func__);
	platform_driver_unregister(&bcm4359_bluetooth_platform_driver);
}

module_init(bcm4359_init);
module_exit(bcm4359_exit);
